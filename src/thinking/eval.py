...
def generate_with_thinking_budget(
    ...
    Phase 2: If delimiter not found, force append it. Then generate remaining response tokens.
             If delimiter was found and tokens were generated after it, subtract those from response_budget.
    ...
    # Phase 1: Generate thinking tokens (don't stop at delimiter, we'll handle overflow)
    ...
    # Process each prompt's outputs
    all_results = []
    prompts_for_phase2 = []
    phase2_mapping = []  # Track which (prompt_idx, sample_idx) each phase2 prompt corresponds to
    remaining_budgets = []  # Track remaining response budget for each sample
    ...
    for prompt_idx, (prompt, thinking_output) in enumerate(zip(prompts, thinking_outputs)):
        ...
        for sample_idx, sample_output in enumerate(thinking_output.outputs):
            ...
            if delimiter_forced:
                ...
                # Delimiter found - find position and count tokens after it
            ...
            # Calculate remaining response budget
            remaining_budget = max(0, response_budget - tokens_after_delim)
    ...
    # Phase 2: Generate response tokens with per-sample budgets
    # Group by remaining budget for efficient batching
    budget_groups = defaultdict(list)
    for idx, budget in enumerate(remaining_budgets):
        budget_groups[budget].append(idx)

    phase2_outputs = [None] * len(prompts_for_phase2)

    for budget, indices in budget_groups.items():
        if budget <= 0:
            # No more tokens needed, create empty outputs
            for idx in indices:
                phase2_outputs[idx] = ""
            continue

        batch_prompts = [prompts_for_phase2[idx] for idx in indices]
        ...
        if lora_request:
            batch_outputs = llm.generate(batch_prompts, response_params, lora_request=lora_request)
        else:
            batch_outputs = llm.generate(batch_prompts, response_params)

        for i, idx in enumerate(indices):
            phase2_outputs[idx] = batch_outputs[i].outputs[0]
