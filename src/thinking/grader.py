...
def compute_score(data_source, solution_str, ground_truth, extra_info=None):
    ...
    """
    #print("[DEBUG GRADER] compute_score called")
    #print(f"[DEBUG GRADER] solution_str length: {len(solution_str)}")
    #print(f"[DEBUG GRADER] Has '</think>' in response: {'</think>' in solution_str}")
    #print(f"[DEBUG GRADER] Has '\\boxed' in response: {r'\\boxed' in solution_str or 'boxed' in solution_str}")
    solution = extract_solution(solution_str)
    #print(f"[DEBUG GRADER] Extracted solution: {solution}")
    #print(f"[DEBUG GRADER] {solution_str=}")
    # If no solution was extracted, return 0 (incorrect)
    if solution is None:
        #print("[DEBUG GRADER] No solution found, returning 0")
        return 0
    if normalize_latex_string(str(ground_truth)) == normalize_latex_string(solution):
        print(f"[DEBUG GRADER] Solution matches ground_truth={ground_truth}, returning 1")
        return 1
    print(f"[DEBUG GRADER] Solution '{solution}' does not match ground_truth='{ground_truth}', returning 0")
    return 0
