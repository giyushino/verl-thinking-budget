...
class vLLMHttpServerBase:
    ...
    async def generate(
        ...
        # Setup lora request (shared by both branches)
        lora_request = None
        if self.model_config.lora_rank > 0:
        ...
        if self.config.thinking_budget > 0:
            ...
            # Get thinking tokens
            thinking_res: Optional[RequestOutput] = None
            ...
            response_res: Optional[RequestOutput] = None
            async for output in generator:
                response_res = output
            ...
            log_probs = None
            if thinking_sampling_params.logprobs is not None:
                thinking_log_probs = [
                    logprobs[thinking_res.outputs[0].token_ids[i]].logprob
                    for i, logprobs in enumerate(thinking_res.outputs[0].logprobs)
                ] if thinking_res.outputs[0].logprobs else []

                # Add placeholder for forced delimiter if it was appended
                if self.config.thinking_delimiter_id not in thinking_res.outputs[0].token_ids:
                    thinking_log_probs.append(0.0)

                response_log_probs = [
                    logprobs[response_res.outputs[0].token_ids[i]].logprob
                    for i, logprobs in enumerate(response_res.outputs[0].logprobs)
                ] if response_res.outputs[0].logprobs else []

                log_probs = thinking_log_probs + response_log_probs

            # Handle routed experts for MoE
            routed_experts = None
            if self.config.enable_rollout_routing_replay:
                # TODO: concatenate routed_experts from both phases if needed
                routed_experts = None

            # Determine stop reason from response phase
            finish_reason = response_res.outputs[0].finish_reason
            ...
            return TokenOutput(
                token_ids=all_token_ids, log_probs=log_probs, routed_experts=routed_experts, stop_reason=stop_reason
            )
